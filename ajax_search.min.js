...
(function(win,doc){
  ...
  function extend(a,b){b=b||{};for(var i in a){if(!is_set(b[i])){b[i]=a[i]}else if(is_object(a[i])&&is_object(b[i])){b[i]=extend(a[i],b[i])}}return b}
  function on(el,ev,fn){el.addEventListener(ev,fn,!1)}
  function off(el,ev,fn){el.removeEventListener(ev,fn)}
  function el(ement,content,attr){ement=doc.createElement(ement);if(is_set(content)&&content!==""){ement.innerHTML=content}if(is_object(attr)){for(var i in attr){if(attr[i]===!1)continue;ement.setAttribute(i,attr[i])}}return ement}
  ...
  function param(o,separator){
    var s=[],i;
    for(i in o){s.push(encode(i)+'='+encode(o[i]))}
    return'?'+s.join(separator||'&')
  }
  function canon(url,x){
    url=(url+"").split(/[?&#]/)[0].replace(/\/+$/,"");
    if(is_set(x)){url=url.replace(/\.[\w-]+$/,x?'.'+x:"")}
    return url
  }
  ...
  function load(url,fn,attr){
    var css=/\.css$/i.test(canon(url)),
        $=el(css?'link':'script',"",extend(css?{'href':url,'rel':'stylesheet'}:{'src':url},attr));
    if($.readyState){
      $.onreadystatechange=function(){
        if($.readyState==="loaded"||$.readyState==="complete"){
          $.onreadystatechange=null;fn&&fn($)
        }
      }
    } else {
      fn&&on($,"load",fn)
  ...
  var text=settings.text,
      ...
      container=el('div','<div></div>',{'class':name+' '+settings.direction,'id':name+':'+hash}),
      div=el('div'),
      title=el('h3',"",{'class':name+'-title'}),
      ol=el('ol',"",{'class':name+'-results','id':name+'-results:'+hash,'start':start}),
      nav=el('nav',"",{'class':name+'-pager','id':name+'-pager:'+hash}),
      previous=el('a',text.previous,{'class':name+'-previous','href':"",'rel':'prev'}),
      next=el('a',text.next,{'class':name+'-next','href':"",'rel':'next'}),
      loading=el('p',text.loading,{'class':name+'-loading'}),
      list;
  
  function _show(){
    if(ad!==!1){
      var i=+(storage.getItem(name)||-1);
      if(i>ad){storage.setItem(name,0);return!0}
  ...
  win['_'+fn]=function($){
    $=$.feed||{};
    var entry=$.entry||[], entry_length=entry.length, i, j, k;
    ...
    list=function(current){
      if(!current)return;
      var url=(current.link.find(function($){return $.rel==="alternate"})||{}).href;
      var str="";
      if(!url)return;
      if(size){
        ...
        if(size===!0) size=80;
        if(is_number(size)){
          var sNum = parseInt(size,10);
          // Pastikan ukuran minimal 250px agar gambar tidak blur
          sNum = sNum < 250 ? 250 : sNum;
          w = h = sNum+'px';
          size = 's'+sNum+'-c';
        } else if(r = /^s(\d+)(\-[cp])?$/.exec(size)){
          var sNum = parseInt(r[1],10);
          sNum = sNum < 250 ? 250 : sNum;
          w = sNum+'px';
          h = r[2] ? sNum+'px' : 'auto';
          size = 's'+sNum+'-c';
        ...
        str += has_image ?
          '<img alt="thumbnail" loading="lazy" src="'+ current.media$thumbnail.url.replace(/\/s\d+(\-c)?\//g, '/'+size+'/') +'" style="display:block;width:'+w+';height:'+h+';">'
          :
          '<span class="img" style="display:block;width:'+w+';height:'+h+';">';
        str += '</p>';
      }
    ...
    if(_show()){
      load(blogger('298900102869691923')+param(extend(settings.query, {'callback':'_'+fn+'_','max-results':21,'orderby':'updated'}))+'&q='+encode(query))
    }
  ...
  function search(q,i){
    ...
    load(blogger(url)+param(extend(settings.query, {'callback':'_'+fn, 'max-results':chunk, 'q':q, 'start-index':i})), function(){
  ...
  var live = settings.live, throttle;
  
  function search_key(e){
    var t = this;
    if(e.key && e.key==='Enter' || e.keyCode && e.keyCode===13){ return }
    win.setTimeout(function(){
      if(live){
        throttle && win.clearTimeout(throttle);
        throttle = win.setTimeout(function(){ search_submit.call(source) }, is_number(live) ? +live : 500)
      } else if(t.value === ""){
        insert(bounds, div);
        detach(container)
      }
